...
with st.sidebar:
    ...
    score_asof_date = st.date_input(
        "Score As-Of Date",
        value=end_date,
        min_value=start_date,
        max_value=end_date,
        help="Recalculate the score using only the data that would have been available up to this date.",
    )
...
score_cutoff = pd.to_datetime(score_asof_date) + timedelta(days=1)
score_df = valid_df[valid_df.index < score_cutoff].copy()

if len(score_df) < 2:
    st.warning("There is not enough valid indicator history inside the selected range to score that as-of date. Choose a later as-of date or widen the range.")
    st.stop()

last_row = score_df.iloc[-1]
prev_row = score_df.iloc[-2]
...
with kpi1:
    st.metric("As-Of Date Price", f"{current_price:.4f}", f"{price_pct:.2f}%")
with kpi2:
...
with col_meta:
    st.caption(f"Scored as of {last_row.name.strftime('%Y-%m-%d')} using only data available up to that date within the selected analysis range.")
...
fig.add_vline(x=last_row.name, line_width=1, line_dash='dash', line_color='#58a6ff')
